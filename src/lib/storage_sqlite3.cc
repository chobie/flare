 ...
 *  storage_sqlite3.cc
 *
 *  implementation of gree::flare::storage_sqlite3
 *
 *  @author Shuhei Tanuma <chobieeee@php.net>
 *
 *  $Id$
 */
#include "app.h"
...
// {{{ ctor/dtor
/**
 *  ctor for storage_sqlite3
 */
storage_sqlite3::storage_sqlite3(string data_dir, int mutex_slot_size, uint32_t storage_ap, uint64_t storage_bucket_size, int storage_cache_size, string storage_compess, bool storage_large, int32_t storage_dfunit):
	storage(data_dir, mutex_slot_size, storage_cache_size) {
	this->_data_path = this->_data_dir + "/flare.db";

	sqlite3_open(this->_data_path.c_str(), &this->_db);
...
int storage_sqlite3::close() {
	if (!this->_open) {
		log_warning("storage is not yet opened", 0);
		return -1;
	}

	if (!sqlite3_close(this->_db)) {
		//BasicDB::Error error = this->_db->error();
		log_err("%s failed: %s (%d)","sqlite3::close()", "todo", 0);
		return -1;
	}
...
int storage_sqlite3::set(entry &e, result &r, int b) {
...
int storage_sqlite3::incr(entry &e, uint64_t value, result &r, bool increment, int b) {
...
int storage_sqlite3::get(entry &e, result &r, int b) {
...
int storage_sqlite3::remove(entry &e, result &r, int b) {
...
int storage_sqlite3::truncate(int b) {
...
int storage_sqlite3::iter_begin() {
...
storage::iteration storage_sqlite3::iter_next(string &key) {
	pthread_mutex_lock(&this->_mutex_iter_lock);
	if (!this->_iter_lock) {
		pthread_mutex_unlock(&this->_mutex_iter_lock);
...
/*
		...
		if (error.code() == BasicDB::Error::NOREC) {
			...
			log_err("storage_sqlite3::iter_next() failed: %s (%d)", error.message(), error.code());
			return iteration_error;
		}
*/
	}
...
uint32_t storage_sqlite3::count() {
	return 0;//return this->_db->count();
}

uint64_t storage_sqlite3::size() {
	return 0;//return this->_db->size();
}

bool storage_sqlite3::is_capable(capability c) {
	return false;
}
// }}}

// {{{ protected methods
int storage_sqlite3::_get_header(string key, entry &e) {
